...
export default class SideBar extends Component {
  ...
  fetchRecommendedUsers = async () => {
    ...
    if (user) {
      try {
        const result = await firebase.default.firestore().collection("users").doc(user.uid).collection("contacts").get();
        let contactsResult = result.docs as firebase.default.firestore.QueryDocumentSnapshot<Contact>[];
        let randomizedFriendIndex = Math.floor(Math.random() * contactsResult.length - 1);
        if (contactsResult[randomizedFriendIndex].data().userUID === user.uid)
           randomizedFriendIndex++;
        const contactData = await firebase.default.firestore().collection("users").doc(contactsResult[randomizedFriendIndex].data().userUID).collection("contacts").get();
        ...
        contactDetails.forEach(async item_two => {
           const friendOfFriendData = await firebase.default.firestore().collection("users").doc(item_two.data().userUID).get();
           const friendOfFriend = friendOfFriendData.data() as User;
           if (friendOfFriend.userUID === user.uid)
               return;
           this.setState({ recommendedUsers: [...this.state.recommendedUsers, friendOfFriend ]});
        });
      }catch(error){
